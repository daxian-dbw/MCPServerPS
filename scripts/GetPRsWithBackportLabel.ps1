...
Retrieves merged pull requests from the PowerShell repository that are marked for backport consider to a specific target release.

.DESCRIPTION

This script queries the PowerShell GitHub repository to find merged pull requests that have been labeled for backport consider to a specified target release version. It uses the GitHub CLI to fetch PR information including number, title, URL, and merge date, then sorts the results from oldest to newest based on merge date.
...
The target PowerShell release version to check for backport consider PRs. Must be one of: '7.4', '7.5', or '7.6'.
...
$Owner = "PowerShell"
$Repo = "PowerShell"
$approvalLabel = "Backport-$TargetRelease.x-Consider"
